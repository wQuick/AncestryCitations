...
class GedcomWriter(UpdateCallback):
    ...
    def _person(self, person):
        ...
        self._names(person)
        self._gender(person)
        self._person_event_ref('BIRT', person.get_birth_ref(), person)
        self._person_event_ref('DEAT', person.get_death_ref(), person)
    ...
    def _process_person_event(self, person, event, event_ref):
        ...
        self._dump_event_stats(event, event_ref, person)
    ...
    def _attributes(self, person):
        ...
        for attr in attr_list:
            ...
            if key.startswith('Ancestry APID H:'):
                continue
            ...
            self._source_references(attr.get_citation_list(), 2, person)

    def _source_references(self, citation_list, level, person=None):
        """
        Loop through the list of citation handles, writing the information
        to the file.
        """
        for citation_handle in citation_list:
            self._source_ref_record(level, citation_handle, person)
    ...
    def _addresses(self, person):
        ...
        for addr in person.get_address_list():
            ...
            self._source_references(addr.get_citation_list(), 2, person)
    ...
    def _person_sources(self, person):
        ...
        for citation_handle in person.get_citation_list():
            self._source_ref_record(1, citation_handle, person)
    ...
    def _person_event_ref(self, key, event_ref, person):
        """
    ...
    def _dump_event_stats(self, event, event_ref, person=None):
        """
        ...
        print('calling _source_references')
        self._source_references(event.get_citation_list(), 2, person)
    ...
    def _source_ref_record(self, level, citation_handle, person):
        """
        ...
        print('person: {} ({})'.format(person, '' if not person else person.get_primary_name().get_gedcom_name()))
        if person:
            apid_db = None
            apid_h = None
            for srcattr in src.get_attribute_list():
                if str(srcattr.type) == 'Ancestry DBID':
                    apid_db = srcattr.value
            if apid_db:
                attr_type = 'Ancestry APID H:{}'.format(apid_db)
                for personattr in person.get_attribute_list():
                    if str(personattr.type) == attr_type:
                        apid_h = personattr.value
                if apid_h:
                    self._writeln(level+1, '_APID', '1,{}::{}'.format(apid_db, apid_h))
